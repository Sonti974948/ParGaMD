...
def compute_cumulants(sum_w: np.ndarray, sum_w_dV: np.ndarray, 
    ...
    if np.any(valid_bins):
        # Safe division: only divide where valid_bins is True
        mean_dV  = np.zeros(shape, dtype=float)
        mean_dV2 = np.zeros(shape, dtype=float)
        mean_dV3 = np.zeros(shape, dtype=float)

        np.divide(sum_w_dV,  sum_w, out=mean_dV,  where=valid_bins)
        np.divide(sum_w_dV2, sum_w, out=mean_dV2, where=valid_bins)
        np.divide(sum_w_dV3, sum_w, out=mean_dV3, where=valid_bins)

        var_dV = mean_dV2 - mean_dV**2

        # 1st cumulant: c1 = beta * <dV>
        c1[valid_bins] = beta * mean_dV[valid_bins]

        # 2nd cumulant: c2 = 1/2 * beta^2 * variance
        c2[valid_bins] = 0.5 * (beta**2) * var_dV[valid_bins]

        # 3rd cumulant: c3 = (1/6)*beta^3*(<dV^3>-3<dV^2><dV>+2<dV>^3)
        c3[valid_bins] = (1.0/6.0) * (beta**3) * (
